...
You must use [these steps]({{ site.data.urls.SkillManualAuth | liquify }}) to manually configure Authentication for the Calendar Skill. Due to a change in the Skill architecture this is not currently automated.
...
Follow the general instructions [here]({{ site.data.urls.SkillManualAuth | liquify }}) to configure this using the scopes shown above.
